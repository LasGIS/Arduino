...
void setup() {
  ...
  oled.roundRect(0, 0, 127, 31, OLED_STROKE);      // прямоугольник (x0,y0,x1,y1)
  ...
  // --------------------------
  oled.setCursorXY(1, 1); // курсор в (пиксель X, пиксель Y)
  oled.setScale(3);
  ...
  delay(2000);
  oled.setContrast(255);   // яркость 0..255
  ...
  oled.rect(15, 15, 35, 30, OLED_STROKE);      // прямоугольник (x0,y0,x1,y1)
  // параметры фигуры:
  // OLED_CLEAR - очистить
  // OLED_FILL - залить
  // OLED_STROKE - нарисовать рамку
  oled.roundRect(50, 5, 80, 25, OLED_STROKE);  // аналогично скруглённый прямоугольник
  oled.circle(60, 15, 15, OLED_STROKE);        // окружность с центром в (x,y, с радиусом)
  oled.circle(60, 15, 5, OLED_FILL);           // четвёртый аргумент: параметр фигуры

  // битмап
  oled.drawBitmap(90, 0, bitmap_32x32, 32, 32, BITMAP_NORMAL, BUF_ADD);
