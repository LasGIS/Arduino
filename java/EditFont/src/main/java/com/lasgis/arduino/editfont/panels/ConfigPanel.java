/*
 *  @(#)ConfigPanel.java  last: 25.08.2024
 *
...
//            portReader.addListener(mainFrame.getMapPanel());
    ...
    public ConfigPanel() {
        ...
        try {
            final FontData fontData = LoadHelper.load();
            filename = fontData.getFileName();
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
        }
        hexFileInput = new JFileChooserField(
            nonNull(filename) ? Path.of(props.getProperty(PROP_PATCH), filename).toString() : props.getProperty(PROP_PATCH),
...
//            portReader.addListener(mainFrame.getMapPanel());
    ...
    private void fillParametersPanel() {
        ...
        gbc.gridwidth = 1;
        gbc.weightx = 0.0;
        parametersPanel.add(createNavigationButton("Create", "create.png", "Создаем по файлу", CommandType.test), gbc);
        ...
        gbc.gridwidth = 1;
        gbc.weightx = 0.0;
        parametersPanel.add(createNavigationButton("Upload", "upload.png", "Запись блока", CommandType.test), gbc);
...
//        gbc.weightx = 0.1;
        ...
        gbc.gridx = 7;
//        gbc.weightx = 0.0;
        parametersPanel.add(createNavigationButton("Read", "userGuide.png", "Чтение блока", CommandType.test), gbc);
